...
class Blockchain {
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            console.log(currentTime < (timeInMessage + 300));
            console.log(bitcoinMessage.verify(message, address, signature))

            if (currentTime < (timeInMessage + (300)) && bitcoinMessage.verify(message, address, signature)) {
                const block = new BlockClass.Block({ address: address, star: star });
            ...
            for (const block of self.chain) {
                ...
                if (block.height === 0) {
                    if (block.previousBlockHash !== null) {
                        errorLog.push(`Invalid expected previous block hash for genesis block`);
                    }
                } else {
                    if (block.previousBlockHash !== self.chain[block.height - 1].hash) {
                        errorLog.push(`Invalid: previous block hash does not match `);
                    }
                }
            }
